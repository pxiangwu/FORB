...
MODEL_PATH = 'models/slip_large_100ep.pt'
...
def main(object_type: str, data_root_path: str, database_name: str, query: bool, database: bool):
    ...
    model = getattr(models, old_args.model)(rand_embed=False, ssl_mlp_dim=old_args.ssl_mlp_dim, ssl_emb_dim=old_args.ssl_emb_dim)
    ...
    if query:
        ...
        for image_path in tqdm.tqdm(query_image_paths):
            ...
            for preprocess in query_preprocess_list:
                ...
                with torch.no_grad():
                    image_features = utils.get_model(model).encode_image(image)
