...
def main(image_path_list: str, num_features: int, workers: int, feature_matrix_path: str, keypoints_path: str,
         descriptors_path: str, resize_dim: int):
    global SIFT
    ...
    for result in extraction_results:
        gcs_uri, keypoints, descriptors, height, width = result
        if descriptors is not None:
            descriptor_count += len(descriptors)
            descriptor_dict[gcs_uri] = descriptors
            keypoint_dict[gcs_uri] = keypoints
            features_dict[gcs_uri] = {'descriptors': descriptors, 'keypoints': keypoints}
            metadata_dict[gcs_uri] = {'height': height, 'width': width}
            features_count_dict[gcs_uri] = len(descriptors)
        else:
            none_count += 1
    ...
    local_features_path = 'artifacts/features.pkl'
    local_metadata_path = 'artifacts/metadata.pkl'
    local_features_count_path = 'artifacts/features_count.pkl'
    ...
    with open(local_features_path, 'wb') as fp:
        pickle.dump(features_dict, fp)
    with open(local_metadata_path, 'wb') as fp:
        pickle.dump(metadata_dict, fp)
    with open(local_features_count_path, 'wb') as fp:
        pickle.dump(features_count_dict, fp)
    del keypoint_dict
    del metadata_dict
    del features_dict
    del features_count_dict
